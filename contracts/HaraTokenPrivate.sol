...
interface IPriceable {
    ...
    // events
    event PriceChangedLog(address indexed itemAddress, bytes32 indexed id, uint oldValue, uint256 newValue);
    ...
    contract HaraTokenPrivate is IBuyMechanism, ERC20Capped(1200000000 * (10 ** uint256(18))) {
    // token details
    ...
    */
    function burnToken(uint256 value) public {    
      emit BurnRequestLog(nonce, msg.sender, value, hashDetails(nonce, msg.sender, value, HART_NETWORK_ID), "");
    ...
    */
    function buy(address seller, bytes32 id, uint256 value, address buyer) public haveEnoughToken(buyer, value) {
        ...
        txReceipt[receiptNonce] = Receipt(buyer, seller, id, value);
        emit ReceiptCreatedLog(receiptNonce, buyer, seller, id, value);

        require(transfer(seller, value), "Payment failed");
        emit PaidLog(buyer, seller, value);
        
        require(item.buy(receiptNonce), "Buy proccess failed");
        emit ItemBoughtLog(receiptNonce, buyer, seller, id, value);
    }
